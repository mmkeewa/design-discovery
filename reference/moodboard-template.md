...
Опциональная текстовая версия борда — собирается **по запросу пользователя** (основной артефакт теперь визуальный борд в Figma). Берёт то, что попало в разделы борда по слоям — без поштучной курации.
...
- [ ] Собрать визуальный борд в Figma (design-файл) — см. `reference/figma-board-build.md`. Единица: раздел → референс (скриншот) → название источника + ссылка
...
1. **Включаем всё, что попало в борд** по трём слоям (после дедупа и отсева явного оффтопа). Поштучной курации нет — synthesis строится по общему в собранном, а не по «выбранным номерам»
